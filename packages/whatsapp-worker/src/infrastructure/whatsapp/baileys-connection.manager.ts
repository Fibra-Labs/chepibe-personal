...
import { whatsappSessions, whatsappSessionKeys, eq } from '@chepibe-personal/shared';
...
export class BaileysConnectionManager {
  ...
  private async loadOrCreateAuthState(sessionId: string): Promise<{
    ...
    const keyStore = new SqliteKeyStore(sessionId, this.db, this.logger);
  ...
  private async handleMessage(m: any, sessionId: string): Promise<void> {
    ...
    for (const message of m.messages) {
      ...
      if (audioMessage) {
        this.logger.info({ sessionId, phoneNumber, msgId, fromMe: isFromMe, duration: audioMessage.seconds, mimetype: audioMessage.mimetype }, 'Message received → audio → processing');
